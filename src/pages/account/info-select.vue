<template>
  <div class="v-editor">
    ...
    <template v-if="type !== 'readOnly'">
      <a href="javascript:;" v-if="!isEdit||clickType!=='save'" @click="modify(clickType)"><span
        v-if=" (value === '无' || value === '' ) && btnName ==='' ">完善</span><span
        v-else-if="value !== '' && value !== '' && btnName ==='' ">修改</span><span
        v-else-if="btnName !=='' ">{{btnName}}</span></a>
...
<script>
  export default {
    ...
    watch: {
      selectParentId: function (newQuestion) {
        this.selectedOptions[0] = newQuestion + ''
        // this.selectedOptions[1] = this.selectChildId + ''
      },
      selectChildId: function (newQuestion) {
        // this.selectedOptions[0] = this.selectParentId + ''
        this.selectedOptions[1] = newQuestion + ''
      }
    }
  }
</script>
