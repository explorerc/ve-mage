<template>
  <div class="account-container">
    ...
    <div class="v-info">
      ...
      <com-editor :value.sync="userName"
                  type="input"
                  @saveInfo="save($event,userName,'name','user')"
                  clickType="save"
                  :max-length="20"
    ...
    <message-box v-show="messageBoxShow"
      ...
      <div v-if="messageBoxType === 'changeMobile' && (step === 'initialPhone' ||  step === 'newPhone')">
        <p class="v-explain"
          ...
          <i class="iconfont icon-duihao"
             v-if="step === 'newPhone'"></i> {{messageBoxExplain}}
        </p>
        ...
        <com-input :value.sync="phoneCode"
                   ...
                   type="input"
                   :max-length="6"
                   @focus="codeFocus"></com-input>
        <p class="v-error"
           v-if="phoneCodeError">{{phoneCodeTip}}</p>
        <a href="javascript:;"
      ...
      <div v-else-if="messageBoxType === 'changePassword'">
        <com-input :value.sync="dsds"
                   ...
                   type="password"
                   :max-length="30"
                   :errorTips="errorTips.oldPassword"
                   @focus="passwordFocus('oldPassword')"></com-input>
        <div class="v-psd">
          <com-input :value.sync="newPassword"
                     placeholder="请输入新密码"
                     class="v-input"
                     type="password"
                     :max-length="30"
                     @focus="passwordFocus('newPassword')"
                     @change="passwordChange()"
                     @blur="passwordBlur()"
                     :errorTips="errorTips.newPassword"></com-input>
        ...
        <com-input :value.sync="reNewPassword"
                   placeholder="请再次输入新密码"
                   class="v-input"
                   type="password"
                   :max-length="30"
                   :errorTips="errorTips.rePassword"
                   @focus="passwordFocus('reNewPassword')"></com-input>
      </div>
...
export default {
  ...
  mounted () {
    ...
    if (contactInfo && contactInfo.userName) {
      ...
      this.$get(userService.GET_USERINFO).then((res) => {
